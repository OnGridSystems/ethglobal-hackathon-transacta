...
try {
  ...
  Contracts = {
    5: {
      Token: GoerliToken,
      Bridge: GoerliBridge,
      BridgeAddress: GoerliBridgeAddress,
    },
    280: {
      Token: ZkSyncToken,
      Bridge: ZkSyncBridge,
      BridgeAddress: ZkSyncBridgeAddress,
    },
    534353: {
      Token: ScrollToken,
      Bridge: ScrollBridge,
...
export async function bridgeToken(
  ...
  try {
    const transfer = await Bridge.bridgeToL2(toChainId, tokenId, {
      from: signerAddress,
    ...
    const testTokens = async () => {
      try {
        ...
        const token = res.find((tx) => tx.token_id === tokenId);
